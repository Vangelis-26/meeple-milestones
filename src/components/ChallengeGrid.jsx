...
export default function ChallengeGrid({ items, loading, onAddClick, onUpdateProgress, onRemoveGame }) {
      ...
      <div className="mt-8">
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((item) => (
               ...
               <div
                  key={item.game_id}
                  className="group relative p-5 rounded-xl shadow-xl flex flex-col gap-5 overflow-hidden bg-[#d4c4a9] border-2 border-[#bfa88a]"
                  style={{
                     // L'image de texture bois
               ...
               >
                  <button
                     onClick={() => onRemoveGame(item.game_id)}
                     className="absolute top-2 right-2 text-[#8b5a2b] opacity-0 group-hover:opacity-50 hover:!opacity-100 transition-opacity p-1"

...
export default function GameCard({ item, gameInfo, onRemove, onClickDetails, onRequestAddPlay }) {
   ...
   return (
      <div className="aspect-[3/4] bg-white border border-stone-200 rounded-xl shadow-md overflow-hidden flex flex-col relative group hover:shadow-xl transition-all w-full">
         ...
         <div className="flex-1 flex flex-col px-3 pb-3 pt-2 bg-stone-50">
            ...
            {/* PLATEAU GRIS & MEEPLES INTERACTIFS */}
            <div className="bg-stone-200 w-fit mx-auto rounded-xl p-4 shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] grid grid-cols-5 gap-x-3 gap-y-4 border border-stone-300/50">
               {Array.from({ length: 10 }).map((_, index) => {
                  ...
                  const isNext = boxNumber === playsCount + 1; // Le seul qu'on peut ajouter

                  return (
                     <button
                        key={boxNumber}
                        // Seuls les remplis (historique) ou le suivant (ajout) sont cliquables
                        disabled={!isFilled && !isNext}

                        onClick={(e) => {
                           e.stopPropagation();
                           // Si c'est le SUIVANT : On ouvre la modale d'ajout
                           if (isNext) {
                              onRequestAddPlay(item, boxNumber);
                           }
                           // Si c'est REMPLI : On ouvre l'historique (via le flag true)
                           else if (isFilled) {
                              onRequestAddPlay(item, null, true);
                           }
                        }}

                        className={`
                           w-9 h-9 rounded-lg flex items-center justify-center relative transition-all duration-200 focus:outline-none
                           ${(isFilled || isNext) ? 'cursor-pointer hover:scale-105' : 'cursor-not-allowed opacity-40 grayscale'}
                           ${!isFilled
                              ? 'bg-stone-300/50 shadow-inner'
                              : 'bg-white shadow-md border border-stone-100 scale-110 z-10 ring-1 ring-black/5'
                           }
                        `}
                        title={isFilled ? "Voir historique" : isNext ? "Enregistrer cette partie" : "Terminez les précédents d'abord"}
                     >
                        <svg
                           viewBox="0 0 512 512"

// 🎨 PALETTE "10 JOUEURS - ÉDITION BOIS" (Couleurs mates et lisibles)
const MEEPLE_COLORS = {
...
export default function GameCard({ item, gameInfo, onUpdateProgress, onRemove }) {
   ...
   return (
      <div className="aspect-[3/4] bg-white border border-stone-200 rounded-xl shadow-md overflow-hidden flex flex-col relative group hover:shadow-xl transition-all w-full">
         ...
         <button
            ...
            className="absolute top-2 right-2 z-30 p-2 bg-white/90 backdrop-blur-sm rounded-full text-stone-400 hover:text-red-600 hover:bg-red-50 shadow-md opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
         ...
         {/* ZONE IMAGE */}
         <div className="h-36 bg-gradient-to-br from-stone-50 to-stone-200 relative overflow-hidden border-b border-stone-100 flex items-center justify-center shrink-0">
            {gameInfo.thumbnail_url ? (
               <a href={`https://boardgamegeek.com/boardgame/${gameInfo.bgg_id}`} target="_blank" rel="noopener noreferrer"
                  className="w-full h-full flex items-center justify-center p-3 z-10 transition-transform duration-500 hover:scale-105"
                  title="Voir sur BGG"
               >
                  <img
                     src={gameInfo.thumbnail_url}
                     alt={gameInfo.name}
         ...
         <div className="flex-1 flex flex-col px-3 pb-3 pt-2">
            ...
            {/* TITRE */}
            <div className="h-10 flex items-center justify-center text-center px-1 mb-1 shrink-0">
               <h3 className="text-amber-800 font-bold font-serif text-lg leading-tight line-clamp-2 tracking-wide">
                  {gameInfo.name}
               </h3>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-end">
               <div className="bg-stone-50 rounded-lg p-2 border border-stone-100/50 flex flex-col h-full justify-between">
                  ...
                  {/* PLATEAU GRIS & MEEPLES */}
                  <div className="bg-stone-200 w-fit mx-auto rounded-xl p-4 shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] grid grid-cols-5 gap-x-3 gap-y-4 border border-stone-300/50 mt-auto">
